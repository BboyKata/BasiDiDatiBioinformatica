...
\end{lstlisting}
darà tutti i risultati tranne Padova.
...
\textbf{Attenzione:} quando non mettiamo \textit{DISTINCT} è come specificare implicitamente che mettiamo la clausola \textit{ALL} che include tutti i valori anche ripetuti.
...
\textbf{INTERROGAZIONI CON RAGGRUPPAMENTO:} sono interrogazioni che utilizzano la clausola \textbf{GROUP BY}.
...
\textbf{ATTENZIONE:} SQL impone che in una interrogazione in cui si fa uso del \textit{GROUP BY}, possono comparire come argomento della \textit{SELECT} solamente un sottoinsieme degli attributi utilizzati per il raggruppamento ed eventuali funzioni di aggregazione (MIN, MAX, COUNT, ...) valutate sugli altri attributi. 
...
\textbf{Esempio:} visualizzare il numero degli studenti che provengono da ogni città, visualizzando anche la città.
\begin{lstlisting}[style=sqlstyle]
