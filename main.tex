...
\textbf{Esercizio 2:} Progettare la base di dati per gestire le regate di una competizione. Ogni regata è identificata da nome e data e caratterizzata dal luogo e dalla descrizione del percorso.

Le barche concorrenti sono caratterizzate dal nome, dalla nazionalita, dal modello, dalla lunghezza e dalla stazza. Si suppone che nome e nazionalità identifichino la barca. Le barche sono finanziate da sponsor, e cioè persone delle quali si conoscono il nome, i cognome, il codice fiscale, l'indirizzo, il telefono e l'ammontare della somma fornita Le barche partecipano alle regate e per ogni regata a cui una barca ha partecipato si conosce il punteggio conseguito.

Ogni barca ha un equipaggio composto da almeno 15 membri, ciascuno con un propio ruolo, che può variare di volta in volta. I membri dell'equipaggio sono persone caratterizzate anche dal numero di vittorie conseguite e appartengono ad un certo equipaggio per un certo periodo di cui si conoscono la data di inizio e la data di fine.

\begin{figure}[h!]
	\centering
	\includegraphics[width=1\linewidth]{er10.png}
\end{figure}

\textbf{Esercizio 6}
Si vuole realizzare la base di dati per la gestione dei film di una casa di produzione cinemategrafica. Per ogni attore sono noti il codice fiscale, il nome, il cognome, il numero di telefono, l'indirizzo di casa e il numero di cellulare. (se disponibile).

Ogni film è caratterizzate da un codice (univoco), un titolo, dal nome del regista e dalla data di inizio di realizzazione. Per i film conclusi sono noti la data di fine produzione e la spesa totale affrontata per produrlo. Per ogni film sono noti la data di fine produzione e la spesa totale affrontata per produrlo. Per ogni film sono noti gli attori che vi hanno preso parte, il ruolo da loro interpretato e il loro compenso. Si supponga che ogni attore possa svolgere al massimo un ruolo per ogni film. Ad ogni film sono associate diverse versioni del copione. Per ogni versione sono noti la data in cui è stata scritta, i nomi degli autori e il testo. I set cinematografici presso i quali si svolgono le riprese sono identificati da un codice univoco e sono caratterizzati da un nome. All'inizio delle riprese di un film occorre aver prenotato i set necessari per le riprese. Per ogni prenotazione si tiene traccia del set cinematografico prenotato. Lo stesso set può essere usato più volte durante le riprese dello stesso film. Si tiene anche traccia di quali attrezzature per gli effetti speciali siano messe a disposizione sui set. Le attrezzature per gli effetti speciali sono identificate univocamente da un codice numerico e sono caratterizzate da un nome e da una descrizione.

\begin{figure}[h!]
	\centering
	\includegraphics[width=1\linewidth]{er11}
	\label{fig:er11}
\end{figure}

\newpage
\textbf{Esercizio 5:}
Si richiede di progettare la base di dati per una catena di supermercati. Ogni supermercato è identificato da un codice ed è caratterizzato da indirizzo, città, numero di telefono e nome del responsabile. I supermercati della catena rilasciano delle carte fedeltà. Ogni carta fedeltà è caratterizzata de un codice univoco, dal cliente titolare della carta e dai punti accummilati. Per ogni cliente titolare di una carta fedeltà si tiene traccia delle seguenti informazioni: codice fiscale, nome, cognome, indirizzo e recapito telefonico (facoltativo). 
...
\begin{esempio}
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.5\linewidth]{er15.pdf}
	\end{figure}
...
\begin{itemize}
	\item \textbf{Collasso verso l'ALTO}: Accorpamento delle entità figlie nelle entità genitore.
	inserisci esempio er
	\item \textbf{Collasso verso il BASSO:} Accorpamento dell'entità genitore nelle entità figlie. 
	\item \textbf{Sostituzione della generalizzazione con delle associazioni.}
\end{itemize}
\subsubsection{Partizionamento/Accorpamento di entità e relazioni}
Si decide se partizionare o accorpare concetti.
Inserisci er e spiega. 

Eliminazione degli atribbuti multivalore.
inserisci er
